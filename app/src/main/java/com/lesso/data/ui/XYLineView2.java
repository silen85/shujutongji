 ...
 */
public class XYLineView2 extends View {
    ...
    public XYLineView2(Context context) {
        super(context);
    }

    public XYLineView2(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public XYLineView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (data.length == 0 || field.length == 0) {
            ...
            for (int i = 0; i < field.length; i++) {
                canvas.drawLine(mPaddingLeft + mSingleWidth * i, 0,
                        mPaddingLeft + mSingleWidth * i, mHeight, mPaint);
            }
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (i == data.length - 1) {
                    ...
                    float x1 = mPaddingLeft + mSingleWidth * i;
                    float y1 = (data[i] / dataUnit) * (mHeight / (countY - 1));

                    if (i == 0) {
                        canvas.drawText(Integer.valueOf((int) data[i]).toString(), x1, mHeight - y1 - mTextHeight / 3, mPaint);
                    } else {
                        canvas.drawText(Integer.valueOf((int) data[i]).toString(), x1, (data[i - 1] <= data[i] || data[i] < dataUnit / 2.8 ? mHeight - y1 - mTextHeight / 3 : mHeight - y1 + mTextHeight / 1.5f), mPaint);
                    }
                    mPaint.setColor(getResources().getColor(colors[i % colors.length]));
                    canvas.drawCircle(x1, mHeight - y1, 10, mPaint);

                } else {

                    float x1 = mPaddingLeft + mSingleWidth * i;
                    float y1 = (data[i] / dataUnit) * (mHeight / (countY - 1));
                    float x2 = mPaddingLeft + mSingleWidth * (i + 1);
                    float y2 = (data[i + 1] / dataUnit) * (mHeight / (countY - 1));

                    mPaint.setStrokeWidth(1.5f);
                    canvas.drawLine(x1, mHeight - y1, x2, mHeight - y2, mPaint);
                    canvas.drawText(Integer.valueOf((int) data[i]).toString(), x1, (data[i] > data[i + 1] || data[i] < dataUnit / 2.8 ? mHeight - y1 - mTextHeight / 3 : mHeight - y1 + mTextHeight / 1.5f), mPaint);
